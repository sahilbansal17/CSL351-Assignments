...
int main(){
    ...
    // Apply Dijkstra separately having all elements in the first column in the priority queue initially, then take the minimum of the shortest distances obtained in the last column

    HeapOfDistanceAndCoordinates my_heap;

    rep (i, n) {
        shortest_dist[i][0] = mat[i][0];
        my_heap.insert({shortest_dist[i][0], {i, 0}});
    }
    ...
    while (!my_heap.isEmpty()) {
        ...
        x = cur_coordinate.F;
        y = cur_coordinate.S;
        // can move either to the right, up or down
    ...
    rep (i, n) {
        if (shortest_dist[i][n - 1] < res) {
            res = shortest_dist[i][n - 1];
        }
    }
